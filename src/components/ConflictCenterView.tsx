      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <h2 className="text-xl font-bold text-slate-900 tracking-tight">Conflict Detection & Resolution Center</h2>
            {conflicts.length === 0 ? (
        ...
        {conflicts.length > 0 && (
          <button
            ...
            disabled={isSolving}
            className="px-5 py-2.5 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 rounded-lg transition-colors shadow-xs flex items-center gap-2"
          >
        ...
        <div
          ...
          className={`p-3.5 rounded-xl border text-xs cursor-pointer transition-all ${
            filterType === 'MISSING_PERIODS'
              ...
              : 'bg-white text-slate-700 border-slate-200 hover:bg-blue-50/40'
          }`}
        >
      ...
      <div className="space-y-4">
        {filteredConflicts.length === 0 ? (
          <div className="bg-white rounded-xl border border-slate-200 p-12 text-center space-y-3 shadow-xs">
            ...
            <h3 className="font-bold text-base text-slate-900">Zero Schedule Conflicts</h3>
          ...
          filteredConflicts.map((conflict) => {
            ...
            return (
              <div
                ...
                className={`bg-white rounded-xl border p-5 shadow-xs flex flex-col md:flex-row items-start justify-between gap-5 transition-all ${
                  isCritical ? 'border-rose-300' : 'border-amber-300'
                }`}
              >
                <div className="space-y-2 flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <h4 className="font-bold text-sm text-slate-900">{conflict.title}</h4>
                  </div>
                  ...
                  {conflict.resolutionOptions && conflict.resolutionOptions.length > 0 ? (
                    conflict.resolutionOptions.map((opt, i) => (
                      <button
                        ...
                        className="px-3 py-1.5 text-xs font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors shadow-2xs text-left truncate"
                      >
                        {opt.label}
                      </button>
                    ))
