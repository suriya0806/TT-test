  ...
  return (
    <div id="schedule-config-view" className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-slate-900 tracking-tight">Working Days & Schedule Configuration</h2>
      ...
      {/* Quick Setup Presets */}
      <div className="bg-indigo-50/50 border border-indigo-200 rounded-xl p-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          <span className="text-xs font-bold text-indigo-950 block">Quick Schedule Presets</span>
          <span className="text-[11px] text-indigo-700">Apply standard institution working calendars with one click:</span>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 rounded-lg text-xs font-semibold transition-all cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 pt-4 border-t border-slate-200">
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-xs rounded-lg border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:outline-hidden font-bold text-slate-900"
            />
        ...
        <div className="pt-4 border-t border-slate-200">
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              type="button"
              onClick={addBreak}
              className="px-3 py-1 text-xs font-medium text-indigo-700 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-lg transition-colors flex items-center gap-1"
            >
          ...
          <div className="space-y-2.5">
            {breaks.map((b, idx) => (
              <div
                key={idx}
                className="p-3 bg-slate-50 rounded-lg border border-slate-200 flex flex-wrap items-center gap-4 text-xs"
              >
                ...
                <div className="flex items-center gap-2">
                  <span className="text-slate-500">After Period:</span>
                  <select
                    value={b.afterPeriod}
            ...
            <tbody className="divide-y divide-slate-100">
              {periodTimeline.map((item) => (
                <tr key={item.period} className="hover:bg-slate-50/50">
                  <td className="p-2.5 font-bold text-slate-900">Period {item.period}</td>
                  <td className="p-2.5 font-mono text-slate-700">{item.start}</td>
                  <td className="p-2.5 font-mono text-slate-700">{item.end}</td>
                  <td className="p-2.5 text-slate-600">{periodDurationMinutes} mins</td>
                  <td className="p-2.5">
                    {item.breakAfter ? (
