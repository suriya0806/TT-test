  ...
  return (
    <div id="existing-timetable-view" className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className="px-3 py-2 text-xs font-medium text-rose-700 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-xs flex items-center justify-between text-xs">
          <div>
            <span className="text-slate-500 font-semibold block">Total Allocations</span>
            <span className="text-xl font-bold text-slate-900">{entries.length} slots</span>
          </div>
        ...
        <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-xs flex items-center justify-between text-xs">
          <div>
            <span className="text-slate-500 font-semibold block">Locked Manual Slots</span>
        ...
        <div className="p-4 bg-white rounded-xl border border-slate-200 shadow-xs flex items-center justify-between text-xs">
          <div>
            <span className="text-slate-500 font-semibold block">Faculty Utilization</span>
            <span className="text-sm font-bold text-slate-900">
          ...
          <span
            className={`px-2 py-0.5 rounded text-[11px] font-bold ${
              activeStaffAssigned === staffList.length ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
            }`}
          >
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 shadow-xs space-y-4">
        <h3 className="font-bold text-sm text-slate-900">Quick Slot Assignment Tool</h3>

        <form onSubmit={handleAssignSlot} className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          <div>
            <label className="block text-[11px] font-semibold text-slate-600 mb-1">Target Class</label>
            <select
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-600 mb-1">Period Number</label>
            <select
          ...
          <div className="flex items-end">
            <button
              type="submit"
              className="w-full py-1.5 text-xs font-semibold text-white bg-slate-900 hover:bg-slate-800 rounded-lg transition-colors shadow-xs flex items-center justify-center gap-1.5"
            >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <h3 className="font-bold text-sm text-slate-900">
              Interactive Grid: <span className="text-indigo-600">{currentClass?.name}</span>
            </h3>
          ...
          <table className="w-full text-xs text-center border-collapse">
            <thead className="bg-slate-50 text-slate-700 border-b border-slate-200 font-bold">
              <tr>
                ...
                {Array.from({ length: scheduleConfig.periodsPerDay }, (_, i) => (
                  <th key={i} className="p-3 border-l border-slate-200">
                    P{i + 1}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-slate-200">
              {scheduleConfig.workingDays.map((day) => (
                <tr key={day} className="hover:bg-slate-50/50">
                  <td className="p-3 text-left font-bold text-slate-800 bg-slate-50/70">{day}</td>
                  {Array.from({ length: scheduleConfig.periodsPerDay }, (_, i) => {
                    const p = i + 1;
                    ...
                    return (
                      <td key={p} className="p-2 border-l border-slate-200 align-top min-w-[120px]">
                        {entry ? (
                          <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-2 text-left relative group">
                            <div className="font-bold text-indigo-950 font-mono text-[11px]">
                              {sub?.code || entry.subjectId}
                            </div>
                            <div className="text-[11px] text-slate-700 truncate font-medium">
                              {staff?.name || entry.staffId}
                            </div>
                            <div className="text-[10px] text-slate-400 truncate">{entry.room || ''}</div>
                            <button
                              type="button"
                              onClick={() => handleClearSlot(day, p)}
                              className="absolute top-1 right-1 text-slate-400 hover:text-rose-600 opacity-0 group-hover:opacity-100 transition-opacity p-0.5"
                              title="Clear slot"
                            >
                          ...
                          <div
                            ...
                            className="h-14 border border-dashed border-slate-200 rounded-lg flex items-center justify-center text-[10px] text-slate-400 hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50/40 cursor-pointer transition-all"
                          >
                            + Assign
                          </div>
                        )}
